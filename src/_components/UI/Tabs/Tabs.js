...
const Tabs = ({ hasNav = false, children }) => {
    ...
    return (
        <div className="tab-container flex justify-between items-center w-full">
            {hasNav ? (
                buttonStatus.leftBtn && (
                    <Button variant='ghost' size='xs' width='none' onClick={handleLeftScroll}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-arrow-left"><circle cx="12" cy="12" r="10" /><path d="M16 12H8" /><path d="m12 8-4 4 4 4" /></svg>
                    </Button>
                )
            ) : null}

            <div class={`${hasNav ? 'scrollable-tab-box grow-1 w-4' : '' }`}>
            ...
            {hasNav ? (
                buttonStatus.rightBtn && (
                    <Button variant='ghost' size='xs' width='none' onClick={handleRightScroll}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-arrow-right"><circle cx="12" cy="12" r="10" /><path d="M8 12h8" /><path d="m12 16 4-4-4-4" /></svg>
                    </Button>
                )
            ) : null}
        </div>
...
const TabItem = ({ isActive, onClick, children, ...props }) => {
    ...
    const className = `tab-item py-1 px-2 my-1 mx-2 rounded-md cursor-pointer border text-sm ${isActive ? 'bg-accent text-custom' : 'bg-another hover-custom'}`;
    return (
